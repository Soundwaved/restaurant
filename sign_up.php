...
if(isset($_POST['username'], $_POST['password'], $_POST['passverif'], $_POST['email'], $_POST['phone']) and $_POST['username']!='')
{
	...
	if($_POST['password']==$_POST['passverif'])
	{
		...
		if(strlen($_POST['password'])>=6)
		{
			...
			if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
			{
				...
				$avatar = mysql_real_escape_string($_POST['phone']);
				...
				if($dn==0)
				{
					...
					if(mysql_query('insert into users(id, username, password, email, phone, signup_date) values ('.$id.', "'.$username.'", "'.$password.'", "'.$email.'", "'.$phone.'", "'.time().'")'))
					{
...
<div class="content">
    <form action="sign_up.php" method="post">
        ...
        <div class="center">
            ...
            <label for="phone">telephone<span class="small">(facultatif)</span></label><input type="text" name="phone" value="<?php if(isset($_POST['phone'])){echo htmlentities($_POST['phone'], ENT_QUOTES, 'UTF-8');} ?>" /><br />
